...
// Manual signed URL generation for OBS (AWS S3 compatible)
async function createSignedUrl(bucket: string, key: string, expires: number, accessKey: string, secretKey: string, endpoint: string): Promise<string> {
  const date = new Date()
  ...
  const canonicalQuerystring = `X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=${accessKey}%2F${dateStamp}%2Fcn-north-4%2Fs3%2Faws4_request&X-Amz-Date=${dateString}&X-Amz-Expires=${expires}&X-Amz-SignedHeaders=host`
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { fileName, expires = 3600 } = await req.json()
    ...
    // Generate signed download URL manually
    const signedUrl = await createSignedUrl(bucket, fileName, expires, accessKeyId, secretAccessKey, endpoint)
    ...
    console.error('Generate download URL function error:', error)
    return new Response(JSON.stringify({
